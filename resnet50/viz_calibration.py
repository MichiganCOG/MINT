...
title_name =  'Orig. ResNet50 ECE: %.4f'%(ece)
#title_name =  'MINT ResNet56 ECE: %.4f'%(ece)
#plt.title(title_name, fontsize=32)
#plt.title('Orig. ResNet50', fontsize=38)
plt.title('MINT ResNet50', fontsize=38)
#plt.title('MINT ResNet56 - CIFAR10 ECE: '+str(ece), fontsize=22)
plt.tick_params(labelsize=30)
plt.setp(ax.spines.values(), linewidth=1, color ='k')
plt.xlim((0,1))
plt.ylim((0,1))
#fig.savefig('./calibration_orig_resnet50.pdf', format='pdf', dpi=1200)
fig.savefig('./calibration_mint_resnet50.pdf', format='pdf', dpi=1200)
#plt.show()
