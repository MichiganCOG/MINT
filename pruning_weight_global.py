...
def pruner(prune_percent, parent_key, children_key, final_weights, model, testloader, device):
    # Create a copy
    ...
    for looper_idx in range(len(parent_key)):
...
def calc_perf(parent_key, children_key):
    ...
    for prune_percent in np.arange(0.0, 1.0, step=0.05):
        acc, true_prune_percent = pruner(prune_percent, parent_key, children_key, init_weights, model, testloader, device)
        if perf is None:
            perf      = [acc]
...
if __name__=='__main__':
    ...
    perf, prune_per = calc_perf(parent_key, children_key)
