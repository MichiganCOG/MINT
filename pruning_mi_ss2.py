...
def activations(data_loader, model, device, item_key, ss_idxs):
    parents_op  = None
...
def calc_perf(parent_key, children_key, alg, clusters=2):
    ...
    for looper_idx in np.arange(0,50000,5000):
        ss_idxs.extend(np.random.choice(np.arange(looper_idx, looper_idx+5000), 1000, replace=False).tolist())
    ...
    for num_layers in range(len(parent_key)):
        ...
        for item_key in unique_keys:
            act[item_key] = activations(trainloader, model, device, item_key, ss_idxs)
        ...
        if alg == '1a':
            ...
            alg1b(num_layers, children, I_parent, p1_op, c1_op)
