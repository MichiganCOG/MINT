...
def calc_perf(model, dataset, parent_key, weights_dir, cores, dims):
    ...
    for item_idx in range(len(parent_key)):
        ...
        p1_op[str(item_idx)] = np.maximum(p1_op[str(item_idx)], 0)
        if len(p1_op[str(item_idx)].shape) > 2:
            p1_op[str(item_idx)] = p1_op[str(item_idx)].reshape(p1_op[str(item_idx)].shape[0], -1)
...
if __name__=='__main__':
    ...
    #parents  = ['conv1.weight', 'conv2.weight', 'conv3.weight', 'conv50.weight']
    parents  = ['conv20.weight',
                'conv21.weight','conv22.weight','conv23.weight','conv24.weight','conv25.weight','conv26.weight','conv27.weight','conv28.weight','conv29.weight', 'conv30.weight',
                'conv31.weight','conv32.weight','conv33.weight','conv34.weight','conv35.weight','conv36.weight','conv37.weight']
    parents  = ['conv1.weight','conv2.weight','conv3.weight','conv4.weight','conv5.weight','conv6.weight','conv7.weight','conv8.weight','conv9.weight', 'conv10.weight',
    ...
    MI_X_M = (h_upper-hM_given_X)*nats2bits
    MI_M_Y = (h_upper-hM_given_Y)*nats2bits
    
    plt.plot(MI_M_Y, MI_X_M)
    plt.show()
